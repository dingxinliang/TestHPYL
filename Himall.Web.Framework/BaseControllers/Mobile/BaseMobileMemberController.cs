...
namespace Himall.Web.Framework
{
    ...
    public abstract class BaseMobileMemberController : BaseMobileTemplatesController
    {
        ...
        {
            ((dynamic)base.ViewBag).AreaName = string.Format("m-{0}", base.PlatformType.ToString());
            if (!filterContext.IsChildAction)
            {
                if ((base.CurrentUser == null) || base.CurrentUser.Disabled)
                {
                    ...
                    if (WebHelper.IsAjax())
                    {
                        flag = this.ProcessInvalidUser_Ajax(filterContext);
                    }
                    else
                    {
                        flag = this.ProcessInvalidUser_NormalRequest(filterContext);
                    }
                    if (flag)
                    {
                        return;
                ...
                else if (!WebHelper.IsAjax() && this.BindOpenIdToUser(filterContext))
                {
                    return;
                }
        ...
        {
            string str1 = this.GetRouteString("controller").ToLower();
            string str2 = this.GetRouteString("action").ToLower();
            string[] strArray1 = new string[1]
              ...
              {
                "editbranchproducttocart",
                "getbranchcartproducts",
                "clearbranchcartproducts",
                "clearbranchcartinvalidproducts"
              };
            return Enumerable.Contains<string>((IEnumerable<string>)strArray1, str1) && Enumerable.Contains<string>((IEnumerable<string>)strArray2, str2);
        }
        protected string GetRouteString(string key)
        {
            return this.GetRouteString(key, "");
        }
        protected string GetRouteString(string key, string defaultValue)
        {
            object obj2 = base.RouteData.Values[key];
            if (obj2 != null)
            {
