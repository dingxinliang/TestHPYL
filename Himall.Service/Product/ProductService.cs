...
namespace Himall.Service
{
    public class ProductService : ServiceBase, IProductService
    {
        ...
        /// <returns></returns>
        public object GetProductCirclePage(long productId, long? circleId, bool? nextPage)
        {
            bool flag1 = true;
            bool flag2 = true;
            ProductCircleInfo productCircleInfo = new ProductCircleInfo();
            List<ProductCircleInfo> productCircleList = this.GetProductCircleList(productId);
            if (productCircleList.Count <= 0)
                return (object)new
                {
                    ErrorMsg = "还未创建商圈"
                };
            ...
            {
                for (int index = 0; index < productCircleList.Count; ++index)
                {
                    if (productCircleList[index].CircleId == circleId.Value)
                    {
                        if (nextPage.HasValue)
                        {
                            if (index == productCircleList.Count - 1)
                            {
                                productCircleInfo = productCircleList[productCircleList.Count - 1];
                                flag2 = false;
                            }
                            else
                            {
                                productCircleInfo = productCircleList[index + 1];
                                if (index + 1 == productCircleList.Count - 1)
                                    flag2 = false;
                            }
                        }
                        else if (index == 0)
                        {
                            productCircleInfo = productCircleList[0];
                            flag1 = false;
                        }
                        else
                        {
            ...
            {
                productCircleInfo = Enumerable.FirstOrDefault<ProductCircleInfo>((IEnumerable<ProductCircleInfo>)productCircleList);
                if (productCircleList.Count == 1)
                {
                    flag1 = false;
                    flag2 = false;
                }
            ...
            return (object)new
            {
                ProductId = productCircleInfo.ProductId,
                CircleId = productCircleInfo.CircleId,
                Description = productCircleInfo.Description,
                MobileDescription = productCircleInfo.MobileDescription,
                IsPrev = flag1,
                IsNext = flag2
            };
        }
        ...
        /// <param name="productId"></param>
        /// <returns></returns>
        public List<ProductCircleInfo> GetProductCircleList(long productId)
        {
            return Enumerable.ToList<ProductCircleInfo>((IEnumerable<ProductCircleInfo>)Queryable.Where<ProductCircleInfo>((IQueryable<ProductCircleInfo>)this.Context.ProductCircleInfo, (Expression<Func<ProductCircleInfo, bool>>)(o => o.ProductId == (long?)productId)));
        }
        /// <summary>
        /// 读取产品商圈信息
        /// </summary>
        /// <param name="circleId"></param>
        /// <returns></returns>
        public ProductCircleInfo GetProductCircleInfo(long circleId)
        {
            return Queryable.FirstOrDefault<ProductCircleInfo>((IQueryable<ProductCircleInfo>)this.Context.ProductCircleInfo, (Expression<Func<ProductCircleInfo, bool>>)(p => p.CircleId == circleId));
        }
        /// <summary>
        /// 保存产品商圈
        /// </summary>
        /// <param name="model"></param>
        public void SubmitProductCircle(ProductCircleInfo model)
        {
            Entities context = this.Context;
            ProductCircleInfo productCircleInfo = Queryable.FirstOrDefault<ProductCircleInfo>((IQueryable<ProductCircleInfo>)context.ProductCircleInfo, (Expression<Func<ProductCircleInfo, bool>>)(p => p.CircleId == model.CircleId));
            if (productCircleInfo == null)
            {
                ProductCircleInfo entity = model;
                entity.CreateTime = new DateTime?(DateTime.Now);
                entity.UpdateTime = new DateTime?(DateTime.Now);
                context.ProductCircleInfo.Add(entity);
            }
            else
            {
                productCircleInfo.ProductId = model.ProductId;
                productCircleInfo.Description = model.Description;
                productCircleInfo.MobileDescription = model.MobileDescription;
                productCircleInfo.UpdateTime = new DateTime?(DateTime.Now);
            }
            context.SaveChanges();
        }
        /// <summary>
        ...
        /// </summary>
        /// <param name="id"></param>
        public void DeleteProductCircle(long id)
        {
            this.Context.Database.ExecuteSqlCommand("delete from Himall_ProductCircle where CircleId = " + (object)id);
        }
        /// <summary>
        /// 更新
        /// </summary>
        ...
        /// <returns></returns>
        public bool UpdateStockBySkuId(string skuId, long stockChange)
        {
            SKUInfo skuInfo = Queryable.FirstOrDefault<SKUInfo>((IQueryable<SKUInfo>)this.Context.SKUInfo, (Expression<Func<SKUInfo, bool>>)(item => item.Id == skuId));
            if (skuInfo != null)
                skuInfo.Stock = stockChange;
        ...
        /// <param name="stockChange"></param>
        /// <returns></returns>
        public bool UpdateStockByProId(long proId, int stockChange)
        {
            IQueryable<SKUInfo> source = Queryable.Where<SKUInfo>((IQueryable<SKUInfo>)this.Context.SKUInfo, (Expression<Func<SKUInfo, bool>>)(e => e.ProductId == proId));
            if (Queryable.Count<SKUInfo>(source) == 0)
                throw new HimallException("参数异常,未找到数据");
            foreach (SKUInfo skuInfo in (IEnumerable<SKUInfo>)source)
                skuInfo.Stock = (long)stockChange;
            return this.Context.SaveChanges() > 0;
        }
    }
}
