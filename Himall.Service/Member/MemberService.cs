...
namespace Himall.Service
{
    public class MemberService : ServiceBase, IMemberService
    {
        ...
        public UserMemberInfo GetUserByCache(long userId)
        {
            ...
            UserMemberInfo data = null;
            string key = CacheKeyCollection.Member(userId);
            if (Cache.Exists(key))
            {
                member = Cache.Get<UserMemberInfo>(key);
            }
            if (member == null)
            {
                ...
                if (member != null)
                {
                    data = new UserMemberInfo
                    {
                        ...
                        InviteUserId = member.InviteUserId,
                        LastLoginDate = member.LastLoginDate,
                        Nick = member.Nick,
                        OrderNumber = member.OrderNumber,
                        ParentSellerId = member.ParentSellerId,
                        Password = member.Password,
                        PasswordSalt = member.PasswordSalt,
                        PayPwd = member.PayPwd,
                        PayPwdSalt = member.PayPwdSalt,
                        Photo = member.Photo,
                        Points = member.Points,
                        UserName = member.UserName,
                        TopRegionId = member.TopRegionId,
                        ShareUserId = member.ShareUserId,
                        Sex = member.Sex,
                        ...
                        photo = member.photo,
                        LastConsumptionTime = member.LastConsumptionTime,
                        TotalAmount = member.TotalAmount
                    };
                    data.MemberOpenIdInfo = (from m in member.MemberOpenIdInfo select new MemberOpenIdInfo { AppIdType = m.AppIdType, Id = m.Id, OpenId = m.OpenId, MemberInfo = null, ServiceProvider = m.ServiceProvider, UnionId = m.UnionId, UnionOpenId = m.UnionOpenId, UserId = m.UserId }).ToList<MemberOpenIdInfo>();
                    data.ShippingAddressInfo = (from s in member.ShippingAddressInfo select new ShippingAddressInfo { Address = s.Address, Id = s.Id, IsDefault = s.IsDefault, IsQuick = s.IsQuick, Phone = s.Phone, RegionFullName = s.RegionFullName, RegionId = s.RegionId, RegionIdPath = s.RegionIdPath, ShipTo = s.ShipTo, UserId = s.UserId }).ToList<ShippingAddressInfo>();
                    data.Himall_FavoriteShops = (from h in member.Himall_FavoriteShops select new FavoriteShopInfo { Date = h.Date, Id = h.Id, ShopId = h.ShopId, Tags = h.Tags, UserId = h.UserId }).ToList<FavoriteShopInfo>();
                    data.Himall_BrowsingHistory = (from h in member.Himall_BrowsingHistory select new BrowsingHistoryInfo { BrowseTime = h.BrowseTime, Id = h.Id, MemberId = h.MemberId, ProductId = h.ProductId }).ToList<BrowsingHistoryInfo>();
                    data.Himall_ProductComments = (from h in member.Himall_ProductComments
                                                   select new ProductCommentInfo
                                                   {
                                                       ...
                                                       UserId = h.UserId,
                                                       UserName = h.UserName
                                                   }).ToList<ProductCommentInfo>();
                    data.Himall_MemberIntegral = (from h in member.Himall_MemberIntegral select new MemberIntegral { AvailableIntegrals = h.AvailableIntegrals, HistoryIntegrals = h.HistoryIntegrals, Id = h.Id, MemberId = h.MemberId, UserName = h.UserName }).ToList<MemberIntegral>();
                    data.Himall_MemberIntegralRecord = (from h in member.Himall_MemberIntegralRecord select new MemberIntegralRecord { Id = h.Id, Integral = h.Integral, MemberId = h.MemberId, RecordDate = h.RecordDate, ReMark = h.ReMark, TypeId = h.TypeId, UserName = h.UserName }).ToList<MemberIntegralRecord>();
                    data.Himall_AgentProducts = (from h in member.Himall_AgentProducts select new AgentProductsInfo { AddTime = h.AddTime, Id = h.Id, ProductId = h.ProductId, ShopId = h.ShopId, UserId = h.UserId }).ToList<AgentProductsInfo>();
                    data.Himall_Promoter = (from h in member.Himall_Promoter select new PromoterInfo { ApplyTime = h.ApplyTime, Id = h.Id, PassTime = h.PassTime, Remark = h.Remark, ShopName = h.ShopName, Status = h.Status, UserId = h.UserId }).ToList<PromoterInfo>();
                    int historyIntegral = this.GetHistoryIntegral(data.Id);
                    data.MemberDiscount = this.GetMemberDiscount(historyIntegral);
                }
                if (member != null)
                {
                    Cache.Insert<UserMemberInfo>(key, data, DateTime.Now.AddMinutes(15.0));
                }
            }
            return member;
        ...
        {
            IQueryable<MemberOpenIdInfo> source = Queryable.Where<MemberOpenIdInfo>((IQueryable<MemberOpenIdInfo>)this.Context.MemberOpenIdInfo, (Expression<Func<MemberOpenIdInfo, bool>>)(p => p.UserId == userId && (int)p.AppIdType == (int)appIdType));
            if (!string.IsNullOrWhiteSpace(serviceProvider))
                source = Queryable.Where<MemberOpenIdInfo>(source, (Expression<Func<MemberOpenIdInfo, bool>>)(d => d.ServiceProvider == serviceProvider));
            return Queryable.FirstOrDefault<MemberOpenIdInfo>(source);
        }
        /// <summary>
        /// 根据关联ID获取用户信息
        /// </summary>
        ...
        /// <returns></returns>
        public UserMemberInfo GetMemberByUnionIdOpenId(string UnionId, string openId)
        {
            UserMemberInfo userMemberInfo = (UserMemberInfo)null;
            if (!string.IsNullOrWhiteSpace(UnionId) && !string.IsNullOrWhiteSpace(openId))
            {
                MemberOpenIdInfo memberOpenIdInfo = Queryable.FirstOrDefault<MemberOpenIdInfo>((IQueryable<MemberOpenIdInfo>)this.Context.MemberOpenIdInfo, (Expression<Func<MemberOpenIdInfo, bool>>)(item => item.UnionId == UnionId && item.OpenId == openId));
                if (memberOpenIdInfo != null)
