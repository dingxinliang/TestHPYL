...
$(function () {
    ...
    $('#list').on('click', '.good-del', function () {
        ...
        $.dialog.confirm('您确定要删除' + (name ? ' “' + name + '” ' : ('这' + ($.isArray(ids) ? ids.length : 1) + '件随访')) + '吗？', function () {
            ...
            $.post('DeleteContent', { ids: ids.toString() }, function (result) {
                loading.close();
                if (result.success) {
                    $.dialog.tips('删除随访成功');
...
function deleteFollowContent(ids) {
    $.dialog.confirm('您确定要删除这些随访吗？', function () {
        var loading = showLoading();
        $.post('DeleteContent', { ids: ids.join(',').toString() }, function (result) {
            loading.close();
            if (result.success) {
                $.dialog.tips('删除随访成功');
                var pageNo = $("#list").hiMallDatagrid('options').pageNumber;
    ...
    $("#list").hiMallDatagrid({
        ...
        columns:
            [[
                ...
                { field: "HFC_Content", title: "随访内容", width: 90, align: "center" },
                ...
                { field: "CategoryName", title: "随访分类", width: 90, align: "center" },
                {
                    ...
                    formatter: function (value, row, index) {
                        html = '<span class="btn-a text-left inline-block"><input class="thisday" type="hidden" value="' + row.HFC_Days + '"/><input class="thiscon" type="hidden" value="' + row.HFC_Content + '"/><input class="thiscId" type="hidden" value="' + row.HTP_ID + '"/><input class="thisId" type="hidden" value="' + row.HFC_ID + '"/><input class="thisName" type="hidden" value="' + row.FollowName + '"/>';
