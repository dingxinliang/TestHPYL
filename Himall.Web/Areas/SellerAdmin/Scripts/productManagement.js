...
$(function () {
    ...
    $('#list').on('click', '.good-down', function () {
        ...
        $.dialog.confirm('您确定要停用' + (name ? ' “' + name + '” ' : ('这' + ($.isArray(ids) ? ids.length : 1) + '件科目')) + '吗？', function () {
            ...
            $.post('batchSaleOff', { ids: ids.toString() }, function (result) {
                ...
                if (result.success) {
                    ...
                    $.dialog.alert('停用科目失败!' + result.msg);
            });
        });
    ...
    $('#list').on('click', '.good-up', function () {
        ...
        if (curType == 'saleOff' && AuditOnOff == 0) {
            $.dialog.alert('违规停用的科目，不能再使用!');
            return;
        }
        $.dialog.confirm('您确定要使用' + (name ? ' “' + name + '” ' : ('这' + ($.isArray(ids) ? ids.length : 1) + '件科目')) + '吗？', function () {
            var loading = showLoading();
            $.post('batchOnSale', { ids: ids.toString() }, function (result) {
                loading.close();
                if (result.success) {
                    $.dialog.tips('申请科目使用成功');
                    var pageNo = $("#list").hiMallDatagrid('options').pageNumber;
                    reload(pageNo);
                }
                else
                    $.dialog.alert('申请科目使用失败!' + result.msg);
            });
        });
    });
    ...
    $('#list').on('click', '.good-del', function () {
        var name = $(this).siblings('.thisName').val();
        var ids = $(this).siblings('.thisId').val();
        $.dialog.confirm('您确定要删除' + (name ? ' “' + name + '” ' : ('这' + ($.isArray(ids) ? ids.length : 1) + '件科目')) + '吗？', function () {
            var loading = showLoading();
            $.post('Delete', { ids: ids.toString() }, function (result) {
                ...
                if (result.success) {
                    ...
                    $.dialog.alert('删除科目失败!' + result.msg);
            });
        });
    });
...
function initInfractionSaleOffGrid() {
    ...
    $("#list").hiMallDatagrid({
        ...
        {
            field: "AuditState", title: "违规停用理由", align: "center",
            formatter: function (value, row, index) {
    ...
    $("#list").hiMallDatagrid({
        ...
        },
        {
            ...
            formatter: function (value, row, index) {
                ...
                if (!row.IsLimitTimeBuy) {
                    html += '<a class="good-check" href="edit/' + row.Id + '">编辑</a>';
                }// html += '<a class="good-check" onclick="onSale(' + row.Id + ',\'' + row.Name + '\')">使用</a>';
                html += '<a class="good-down">停用</a>';
                html += '<a class="good-del">删除</a></span>';
                return html;
            }
        }
    ...
    $("#list").hiMallDatagrid({
         ...
         { field: "PublishTime", title: "发布时间", width: 90, align: "center" },
        {
            ...
            formatter: function (value, row, index) {
                ...
                if (row.SaleState == 1) {
                    html += '<a class="good-down">停用</a><a class="good-del">删除</a><div class="good-share" style="display:none"><i style="color:#337ab7">分享</i>';
                    ...
                    html += '<a href="javascript:void(0);" style="display:none" title="分享到微信" onclick="showQrCode(\'' + row.QrCode + '\')"><img src="/Images/wx.png"/></a><s></s></div></div>';
                }

                else { html += '<a class="good-up">使用</a><a class="good-del">删除</a>'; }
                if (row.SaleState == 2 || row.SaleState == 3) {
                    html += '<a style="display:none" style="display:none" class="recommend block2" href="javascript:" data-product-id="{0}" data-relation-products="{1}">推荐科目</a>'.format(row.Id, row.RelationProducts);
                } else {
                    html += '<a class="recommend" style="display:none" href="javascript:" data-product-id="{0}" data-relation-products="{1}">推荐科目</a>'.format(row.Id, row.RelationProducts);
                }
                html += '<a class="good-check" href="CircleList/' + row.Id + '" style="display:none">商圈介绍</a>';
                if (row.IsOverSafeStock) {
                    html += '<span class="red font12 inline-block ml5 mt5" style="display:none"><i class="glyphicon glyphicon-exclamation-sign"></i> 库存已到警戒值</span></span>';
                }
                return html;
            }
        }
...
function saleOff(ids) {
    $.dialog.confirm('您确定要停用这些科目吗？', function () {
        var loading = showLoading();
...
function onSale(ids) {
    $.dialog.confirm('您确定要使用这些科目吗？', function () {
        var loading = showLoading();
        $.post('batchOnSale', { ids: ids.join(',').toString() }, function (result) {
            ...
            if (result.success) {
                ...
                $.dialog.alert('使用科目失败!' + result.msg);
        });
    });
}
