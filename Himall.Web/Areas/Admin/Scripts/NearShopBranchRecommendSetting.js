...
$(function () {
    ...
    $("#recommendShopBranchTable").on("click", '.glyphicon-circle-arrow-up', function () {
        ...
        if (rowIndex > 0) {
            ...
            changeRecommendSequence(oriShopBranchId, newShopBranchId, function () {
                $("#recommendShopBranchTable").hiMallDatagrid('reload', {});
            });
        }
    ...
    $('#recommendBtn').unbind('click').click(function () {
        $("#recommendGrid").hiMallDatagrid('reload', getRecommendQuery());
    });
});
...
function initRecommendShopBranchTable() {
    //门店表格
    $("#recommendShopBranchTable").hiMallDatagrid({
...
function addRecommendShopBranch() {
    ...
    var jumpUrl = "http://" + window.location.host + "/m-wap/ShopBranch/Index/{0}";
    $("#recommendGrid").hiMallDatagrid({
        url: queryUrl,
        nowrap: false,
        rownumbers: true,
