...
function InvoiceInit() {
    ...
    $("#btnAddInvoice").click(function () {
        ...
        html += '公司：<input type="text" value="" id="invoicename" >';
        html += '税号：<input type="text" value="" id="invoicecode">';
    ...
    $("#btnOk").click(function () {
        var title = $("#dvInvoice .invoice-tit-list .invoice-item-selected #invoicename").val();
        var code = $("#dvInvoice .invoice-tit-list .invoice-item-selected #invoicecode");
        var context = $("#dvInvoice .invoice-list .invoice-item-selected span").html();
        if (title == null) {

        }
        else if (title.length > 0 && context.length > 0 && $.trim(title) != "") {
            $("#invoiceTitle").parent().find(".code").remove();
            $("#invoiceTitle").html(title);
            $("#invoiceContext").html(context);
            $('.thickbox,.thickdiv').hide();
            if (code.length > 0 && $.trim(code.val()) != "") {
                $("#invoiceTitle").after("<span class=\"mr15 code\">税号：</span><span id=\"invoiceCode\" class=\"mr15 code\">" + code.val() + "</span>");
            }
        }
        else {
...
function InvoiceOperationInit() {
    ...
    $("#dvInvoice .invoice-tit-list .update-tit").click(function () {
        ...
        var loading = showLoading();
        $.post("./SaveInvoiceTitle", { name: name, code: code }, function (result) {
            loading.close();
        ...
        if (!isSubmitLoading) {
            ...
            if (!recieveAddressId) {
                ...
                $.post('/order/SubmitOrder', {
                    ...
                    invoiceTitle: invoiceTitle, invoiceCode: invoiceCode, invoiceContext: invoiceContext,
                    isCashOnDelivery: isCashOnDelivery, orderShops: orderShops, latAndLng: latAndLng
                },
					function (result) {
...
function ShowPaidPrice() {
    ...
    if (intRule > 0) {
        ...
        var _tmpnum = parseInt(orderTotalPrice * 100 - orderTotalDisPrice * 100) / 100;
    ...
    d_capital.val(useCapital.toFixed(2));
    ...
    d_orduseCapital.html("￥-" + useCapital.toFixed(2));
...
$(function () {
    $('#saveConsigneeTitleDiv').click(function () {
        ...
        if ($('#addressEditArea').css('display') == 'none') {
            ...
            var newSelectedText = '<span>' + selectedAddress.shipTo + '</span> ' + '<br />' + selectedAddress.fullRegionName + ' &nbsp; &nbsp;' + selectedAddress.address + ' &nbsp; &nbsp;' + selectedAddress.addressDetail + '</br>' + selectedAddress.phone;
