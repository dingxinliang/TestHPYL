...
function getIsOpenStore() {
    $.get('/' + areaName + '/Branchproduct/GetIsOpenStore', function (data) {
...
function GetStoresInfo(fromLatLng) {
    ...
    var pid = getProductId();
    $.get('/' + areaName + '/Branchproduct/GetStroreInfo?productId={0}&shopId={1}&fromLatLng={2}'.format(pid, shopId, fromLatLng), function (data) {
        if (data.success == true) {
        ...
        success: function (data) {
            if (data) {
                ...
                if (data.auditStatus == 1) {//冻结
                    ...
                    $("#jd-price .btnBlue").before("<span class=\"btnnobuy\">已下架</span>");
                } else {//正常
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $('.wrap-num .glyphicon-plus').click(function () {
                var pid = getProductId();
                if (canBuy(pid, parseInt($('#buy-num').val()) + 1) == false) {
                    return false;
                }
                $('#buy-num').val(parseInt($('#buy-num').val()) + 1);
                if (checkBuyNum($('#buy-num'))) {
...
function AddCartHtml(obj, buynum) {
    ...
    $(obj).before("<div class=\"wrap-num\" style=\"float:right\">" +
                            "<a class=\"glyphicon glyphicon-minus\" href=\"javascript:;\"></a>" +
                            "<input class=\"input-xs form-control\" id=\"buy-num2\" value=\"" + buynum + "\" readonly=\"readonly\" onkeyup=\"(this.v=function(){this.value=this.value.replace(/[^0-9-]+/,'');}).call(this)\" onblur=\"this.v()\">" +
    ...
    $('.wrap-num .glyphicon-plus').click(function () {
        var pid = getProductId();
        if (canBuy(pid, parseInt($('#buy-num2').val()) + 1) == false) {
            return false;
        }
        $('#buy-num2').val(parseInt($('#buy-num2').val()) + 1);
        if (checkBuyNum($('#buy-num2'))) {
...
function editcart(numObj) {
    ...
    if (len === $(".spec").length) {
        ...
        if (canBuy(pid, numObj.val()) == false) {
            return false;
        }
...
function LoadActives() {
    ...
    $.post(url, { shopid: $("#shopId").val(), productId: $("#gid").val(), branchId: $("#bid").val() }, function (data) {
        var canshow = false;
