        ...
        success: function (data) {
            if (data) {
                ...
                if (data.saleStatus == 1 && data.auditStatus == 2) {
                    if (data.stock > 0) {
                        actionBarHtml += " <button class=\"cell cart\">加入购物车</button><a class=\"btn btn-default\" href=\"../Circle/" + getProductId() + "\">商圈介绍</a><button class=\"cell buy\">立即购买</button>";
                    } else {
...
function loadSkus() {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            // 属性选择弹窗
            $('.att-popup-trigger, .cart, .buy, .limi-btn, .circle').on('click', function (event) {
