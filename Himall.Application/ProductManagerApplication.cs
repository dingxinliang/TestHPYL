...
namespace Himall.Application
{
	public class ProductManagerApplication
	{
        ...
        /// <returns></returns>
        public static object GetProductCirclePage(long productId, long? circleId, bool? nextPage)
        {
            return ProductManagerApplication._productService.GetProductCirclePage(productId, circleId, nextPage);
        }
        /// <summary>
        /// 查询产品类表
        /// </summary>
        /// <param name="productId"></param>
        /// <returns></returns>
        public static List<ProductCircleInfo> GetProductCircleList(long productId)
        {
            return ProductManagerApplication._productService.GetProductCircleList(productId);
        }
        /// <summary>
        /// 查询详细信息
        /// </summary>
        /// <param name="circleId"></param>
        /// <returns></returns>
        public static ProductCircleInfo GetProductCircleInfo(long circleId)
        {
            return ProductManagerApplication._productService.GetProductCircleInfo(circleId);
        }
        /// <summary>
        /// 保存信息
        /// </summary>
        /// <param name="info"></param>
        public static void SubmitProductCircle(ProductCircleInfo info)
        {
            ProductManagerApplication._productService.SubmitProductCircle(info);
        }
        ...
        private static void CreatBrandchWAPHtml(long productId, long branchId)
        {
            WebClient webClient = new WebClient();
            string address = System.Configuration.ConfigurationManager.AppSettings["CurDomainUrl"] + (object)"/m-wap/BranchProduct/Details/" + (string)(object)productId + "?nojumpfg=1&shopBranchId=" + (string)(object)branchId;
            HimallIO.CreateFile("/Storage/Products/Statics/" + (object)productId + "-" + (string)(object)branchId + "-wap-branch.html", (Stream)new MemoryStream(webClient.DownloadData(address)), FileCreateType.Create);
        }
        ...
        {
            string str = "/Storage/Products/Statics/" + (object)productId + "-" + (string)(object)branchId + "-wap-branch.html";
        ...
        {
            return (DateTime.Now - modified).TotalMinutes > (double)minutes;
        }
        ...
        private static void RefreshWapBranchProductHtmlInRemote(long productId, long branchId)
        {
        ...
        /// <returns></returns>
        public static bool BranchCanBuy(long userId, long productId, int count, out int reason)
        {
            ProductInfo product = ProductManagerApplication._productService.GetProduct(productId);
            if ((product.SaleStatus != ProductInfo.ProductSaleStatus.OnSale || product.AuditStatus != ProductInfo.ProductAuditStatus.Audited) && (product.SaleStatus != ProductInfo.ProductSaleStatus.InStock && product.AuditStatus != ProductInfo.ProductAuditStatus.WaitForAuditing))
            {
                reason = 1;
                return false;
            }
            if (Enumerable.Sum<SKUInfo>((IEnumerable<SKUInfo>)product.SKUInfo, (Func<SKUInfo, long>)(p => p.Stock)) == 0L)
            {
                reason = 9;
                return false;
            }
            ...
            Dictionary<long, int> productBuyCount = OrderApplication.GetProductBuyCount(userId, (IEnumerable<long>)new long[1]
          {
            productId
          });
            if (product.MaxBuyCount < count + (productBuyCount.ContainsKey(productId) ? productBuyCount[productId] : 0))
            {
                reason = 3;
                return false;
            }
