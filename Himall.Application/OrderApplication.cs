...
namespace Himall.Application
{
    public class OrderApplication
    {
        ...
        /// <param name="couponIds">诊疗项目对应的优惠券ID集合</param>
        ...
        /// <param name="PlatformType">预约单来源平台</param>
        /// <returns>预约单集合,是否操作成功</returns>
        public static OrderReturnModel SubmitOrder(CommonModel.OrderPostModel submitModel, string payPwd = "")
        {
            if (((submitModel.Capital > 0M) && !string.IsNullOrEmpty(payPwd)) && !MemberApplication.VerificationPayPwd(((UserMemberInfo)submitModel.CurrentUser).Id, payPwd))
            {
            ...
            if (submitModel.OrderShops == null || submitModel.OrderShops.Any(p => p.OrderSkus == null || p.OrderSkus.Any(pp => string.IsNullOrWhiteSpace(pp.SkuId) || pp.Count <= 0)))
                throw new Himall.Core.HimallException("创建预约单的时候，SKU为空，或者数量为0");
        ...
        {
            MemberIntegralExchangeRules integralChangeRule = _iMemberIntegralService.GetIntegralChangeRule();
            MemberIntegral memberIntegral = _iMemberIntegralService.GetMemberIntegral(userid);
            CapitalInfo capitalInfo = _iMemberCapitalService.GetCapitalInfo(userid);
            int num = (integralChangeRule == null) ? 0 : integralChangeRule.MoneyPerIntegral;
            OrderSubmitModel model = new OrderSubmitModel
            {
                IntegralPerMoney = (integralChangeRule == null) ? 0 : integralChangeRule.IntegralPerMoney,
                Integral = (memberIntegral == null) ? 0 : memberIntegral.AvailableIntegrals,
                Capital = ((capitalInfo == null) || !capitalInfo.Balance.HasValue) ? 0 : capitalInfo.Balance.Value,
                Member = MemberApplication.GetMember(userid),
                cartItemIds = cartItemIds
            };
            GetOrderProductsInfo(model, cartItemIds, regionId, userid, cartInfo);
            model.TotalIntegral = (num == 0) ? 0 : Convert.ToInt32(Math.Floor((decimal)(model.totalAmount / num)));
            model.MoneyPerIntegral = num;
            ShippingAddressInfo shippingAddress = GetShippingAddress(regionId, userid);
            model.address = shippingAddress;
            if (shippingAddress != null)
            {
                bool flag = PaymentConfigApplication.IsCashOnDelivery((long)shippingAddress.RegionId);
                model.IsCashOnDelivery = flag;
            }
            else
            {
            ...
            model.InvoiceTitle = _iOrderService.GetInvoiceTitles(userid);
            model.InvoiceContext = _iOrderService.GetInvoiceContexts();
            return model;
        }
        ...
        public static QueryPageModel<FullOrder> GetFullOrders(OrderQuery query)
        {
            ...
            foreach (var order in models)
            {
                ...
                if (ordref != null)
                {
                    ...
                    {
                        order.ShowRefundStats = order.ShowRefundStats.Replace("诊所", "门店");
                    }
                }
                foreach (var item in order.OrderItems)
                {
                    ...
                    if (orditemref != null)
                    {
                        ...
                        if (order.ShopBranchId > 0)
                        {
                            item.ShowRefundStats = item.ShowRefundStats.Replace("诊所", "门店");
                        }
                    }
                }
            }
        ...
        public static OrderSubmitModel SubmitByProductId(long userid, string skuIds, string counts, long? regionId, string collpids = null)
        {
            ...
            submitModel.collIds = collpids;//组合购诊疗项目ID
        ...
        public static long SaveInvoiceTitle(long userid, string name, string code)
        {
            ...
            if (!string.IsNullOrWhiteSpace(info.Name))
                num = OrderApplication._iOrderService.SaveInvoiceTitle(info);
            return num;
        }
        ...
        /// <param name="counts">门店，诊疗项目id和数量的集合</param>
        /// <returns></returns>
        ...
        public static bool PayByCapital(long userid, string orderIds, string pwd, string hostUrl)
        {
            ...
            if (total == 0)
            {
                throw new HimallException("错误的预约单总价");
            }*/
        ...
        public static PaymentViewModel GetPay(long userid, string orderIds, string webRoot)
        {
            ...
            if (orders.Count <= 0)//预约单已经支付，则跳转至预约单页面
            {
                ...
                result.IsSuccess = false;
                if (errorOrder > 0)
                    result.Msg = "预约单已关闭，不能进行支付。";
                else
                    result.Msg = "系统错误，您可以到 “我的预约单” 查看付款操作是否成功。";
                return result;
            }
            else
            {
        ...
        public static ChargePayModel ChargePay(long userid, string orderIds, string webRoot)
        {
            ...
            if (model == null || model.MemId != userid || model.ChargeStatus == ChargeDetailInfo.ChargeDetailStatus.ChargeSuccess)//预约单已经支付，则跳转至预约单页面
            {
        ...
        public static OrderCreateModel GetLimitOrder(CommonModel.OrderPostModel model)
        {
            ...
            if (result.Counts.Count() == 0)
                throw new InvalidPropertyException("待提交预约单的诊疗项目数量不能这空");
        ...
        public static string LimitRedisSubmit(OrderCreateModel model)
        {
            ...
            //    throw new Himall.Core.InvalidPropertyException("创建预约单的时候，SKU为空，或者数量为0");
        ...
        {
            if (model.Capital > new Decimal(0) && !string.IsNullOrEmpty(payPwd) && !MemberApplication.VerificationPayPwd(model.CurrentUser.Id, payPwd))
                throw new HimallException("预付款支付密码错误");
            List<OrderInfo> order = OrderApplication._iOrderService.CreateOrder(model);
            Decimal num = Enumerable.Sum<OrderInfo>((IEnumerable<OrderInfo>)order, (Func<OrderInfo, Decimal>)(item => item.OrderTotalAmount - item.CapitalAmount));
            return new OrderReturnModel()
            {
                Success = true,
                OrderIds = Enumerable.ToArray<long>(Enumerable.Select<OrderInfo, long>((IEnumerable<OrderInfo>)order, (Func<OrderInfo, long>)(item => item.Id))),
                OrderTotal = num
            };
        }
        /// <summary>
        ...
        public static MobileOrderDetailConfirmModel GetMobileSubmit(long userid, string skuIds, string counts)
        {
            if (string.IsNullOrEmpty(skuIds))
                throw new InvalidPropertyException("待提交预约单的诊疗项目ID不能为空");
        ...
        {
            List<ShippingAddressInfo> list = Enumerable.ToList<ShippingAddressInfo>((IEnumerable<ShippingAddressInfo>)ShippingAddressApplication.GetUserShippingAddressByUserId(userid));
            if (shopBranchId > 0)
            {
                ShopBranchInfo shopBranchById = OrderApplication._iShopBranchService.GetShopBranchById(shopBranchId);
                if (shopBranchById == null)
                    return list;
                foreach (ShippingAddressInfo shippingAddressInfo in list)
                {
                    ...
                    if (serveRadius.HasValue)
                    {
                        ...
                        {
                            double distancesFromApi = OrderApplication._iShopBranchService.GetLatLngDistancesFromAPI(fromLatLng, string.Format("{0},{1}", (object)shopBranchById.Latitude, (object)shopBranchById.Longitude));
        ...
        static void GetOrderProductsInfo(long userid, IEnumerable<string> skuIds, IEnumerable<int> counts, MobileOrderDetailConfirmModel confirmModel)
        {
            ...
            foreach (var shopcartItem in shopList)
            {
                ...
                if (shop.IsSelf)
                {
                    //只有官方自营店诊疗项目，才有会员折扣，会员折扣优先级高于满减、优惠券
                    //计算会员折扣
        ...
        public static BaseCoupon GetDefaultCoupon(long shopid, long userid, decimal totalPrice)
        {
            ...
            if (shopBonus.Count() > 0 && userCoupons.Count() > 0)
            {
                ...
                var sb = shopBonus.FirstOrDefault();      //诊所红包
        ...
        /// <returns></returns>
        public static long GetSaleCount(DateTime? startDate = null, DateTime? endDate = null, long? shopBranchId = null, long? shopId = null, long? productId = null, bool hasReturnCount = true)
        {
            return OrderApplication._iOrderService.GetSaleCount(startDate, endDate, shopBranchId, shopId, productId, hasReturnCount);
        }
        public static List<InvoiceTitleInfo> GetInvoiceTitles(long userid)
        {
            return OrderApplication._iOrderService.GetInvoiceTitles(userid);
        }
        #endregion
        ...
        /// 取得诊疗项目描述字符串
        /// </summary>
        /// <param name="orders">诊疗项目对象集合</param>
        ...
        static string GetProductNameDescriptionFromOrders(IEnumerable<OrderInfo> orders)
        {
            ...
            if (productNames.Count > 0)
                productInfos = productNames.Count() > 1 ? (productNames.ElementAt(0) + " 等" + productNames.Count() + "种诊疗项目") : productNames.ElementAt(0);
            return productInfos;
        }
        ...
        {
            ShippingAddressInfo userShippingAddress = new ShippingAddressInfo();
            if (regionId.HasValue)
            {
                userShippingAddress = ShippingAddressApplication.GetUserShippingAddress(regionId.Value);
            }
            else
            {
                userShippingAddress = ShippingAddressApplication.GetDefaultUserShippingAddressByUserId(userid);
            }
            int cityId = 0;
            if (userShippingAddress != null)
            {
                cityId = userShippingAddress.RegionId;
            }
            IEnumerable<ShoppingCartItem> cartItems = null;
            if (string.IsNullOrWhiteSpace(cartItemIds))
            {
            ...
            {
                ICartService _iCartService = ObjectContainer.Current.Resolve<ICartService>();
                cartItems = _iCartService.GetCartItems(from t in cartItemIds.Split(new char[] { ',' }) select long.Parse(t));
            }
            List<CartItemModel> source = GenerateCartItem(cartItems);
            IEnumerable<IGrouping<long, CartItemModel>> enumerable3 = from a in source group a by a.shopId;
            List<ShopCartItemModel> list2 = new List<ShopCartItemModel>();
            IOrderService service = _iOrderService;
            decimal memberDiscount = model.Member.MemberDiscount;
            foreach (IGrouping<long, CartItemModel> grouping in enumerable3)
            {
                IEnumerable<long> productIds = from r in grouping select r.id;
                IEnumerable<int> counts = from r in grouping select r.count;
                ShopCartItemModel item = new ShopCartItemModel
                {
                    shopId = grouping.Key
                };
                Shop shop = ShopApplication.GetShop(item.shopId, false);
                item.CartItemModels = (from a in source
                                       where a.shopId == item.shopId
                                       select a).ToList<CartItemModel>();
                ShopInfo shopInfo = ShopApplication.GetShopInfo(item.shopId, false);
                item.ShopName = shopInfo.ShopName;
                item.FreeFreight = shopInfo.FreeFreight;
                if (cityId > 0)
                {
                    item.Freight = _iProductService.GetFreight(productIds, counts, cityId);
                }
                foreach (CartItemModel model2 in item.CartItemModels)
                {
                    decimal num3 = model2.price * memberDiscount;
                    model2.price = shop.IsSelf ? num3 : model2.price;
                }
                item.FullDiscount = GetShopFullDiscount(item.CartItemModels);
                item.BaseCoupons = GetBaseCoupon(item.shopId, userid, item.ShopTotalWithoutFreight);
                IEnumerable<OrderSubmitItemModel> orderItems = GetOrderItems(item);
                item.freightProductGroup = (from e in orderItems
                                            orderby e.FreightTemplateId
                                            select e).ToList<OrderSubmitItemModel>();
                list2.Add(item);
            }
            model.products = list2;
            model.totalAmount = source.Sum<CartItemModel>(((Func<CartItemModel, decimal>)(item => (item.price * item.count)))) - list2.Sum<ShopCartItemModel>(((Func<ShopCartItemModel, decimal>)(a => a.FullDiscount)));
            model.Freight = list2.Sum<ShopCartItemModel>((Func<ShopCartItemModel, decimal>)(a => a.Freight));
        ...
        static void SetOrderProductsInfo(OrderSubmitModel model, string skuIds, string counts, long userid, string collIds = null)
        {
            ...
            var productService = _iProductService;
            int index = 0;
            var skuCount = skuIdsArr.Length;//有多少个SKU就是多少个诊疗项目
        ...
        static decimal GetSalePrice(long productId, SKUInfo sku, long? collid, int Count, long? userId = null)
        {
            ...
            if (collid.HasValue && collid.Value != 0 && Count > 1)//组合购大于一个诊疗项目
            {
            ...
            else if (Count == 1) //只有一个诊疗项目可能是限时购
            {
