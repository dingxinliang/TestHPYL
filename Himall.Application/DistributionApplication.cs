...
namespace Himall.Application
{
    public class DistributionApplication
    {
        ...
        /// <returns></returns>
        public static bool IsPromoter(long userId)
        {
            return _iDistributionService.IsPromoter(userId);
        }
        ...
        /// <returns></returns>
        public static Himall.DTO.DistributionProducts GetDistributionProductInfo(long productId)
        {
            DistributionProducts result = null;
            var model = _iDistributionService.GetDistributionProductInfo(productId);
            if (model != null)
            {
                var rate = model.rate;
                ...
                if (rate > 0)
                {
                    Commission = model.Himall_Products.MinSalePrice * rate / 100;
                    int _tmp = (int)(Commission * 100);
                    //保留两位小数，但不四舍五入
                    Commission = (decimal)(((decimal)_tmp) / (decimal)100); ;
                }
                result = new Himall.DTO.DistributionProducts
                {
                    Id = productId,
                    Image = Himall.Core.HimallIO.GetProductSizeImage(model.Himall_Products.RelativePath, 1, (int)Himall.CommonModel.ImageSize.Size_100),
                    Price = model.Himall_Products.MinSalePrice,
                    ProductbrokerageId = model.Id,
                    ProductId = model.ProductId.Value,
                    ProductName = model.Himall_Products.ProductName,
                    Status = model.Status,
                    Commission = Commission,
                    AgentNum = model.AgentNum.HasValue ? model.AgentNum.Value : 0
                };
            }
            return result;
        }
        ...
        /// <param name="productid"></param>
        /// <returns></returns>
        private decimal GetDistributionProductAmount(long productid)
        {
            return 1M;
        }
        /// <summary>
